...
class ApiFeatures<T> {
    ...
    constructor(prisma: PrismaClient, model: keyof PrismaClient, queryString: QueryString) {
        this.prisma = prisma;
        this.model = model;
        ...
        Object.keys(queryObj).forEach(key => {
            if (typeof queryObj[key] === 'object') {
                Object.keys(queryObj[key]).forEach(op => {
                    const prismaOp = op === 'gte' || op === 'gt' || op === 'lte' || op === 'lt' ? op : null;
                    if (prismaOp) {
    ...
    limitFields(): this {
        if (this.queryString.fields) {
            this.queryOptions.select = this.queryString.fields.split(',').reduce((acc: any, field: string) => {
                acc[field] = true;
                return acc;
            }, {});
        }
